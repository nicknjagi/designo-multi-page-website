...
const LocationCard = ({country, phone, mail, office, street, address, alt, img}) => {
  return (
    <section id={country} className="flex flex-col lg:flex-row md:gap-6 lg:gap-[30px] lg:h-[326px]">
        <div className="overflow-hidden md:rounded-2xl w-full lg:max-w-[350px] lg:h-full">
            <Image
                src={`/assets/locations/tablet/${img}`}
                width={689}
                height={326}
                className='w-full min-h-[320px] object-cover object-center lg:hidden'
                alt={alt}
            />
            <Image
        ...
        <div className="relative bg-light-peach py-20 md:py-[88px] px-4 md:px-[72px] text-center md:text-left overflow-hidden md:rounded-2xl w-full">
            <h2 className="text-peach-primary text-[32px] md:text-[40px] font-medium mb-6">{country}</h2>
            <div className="flex flex-col md:flex-row gap-6 md:gap-36 lg:gap-24 xl:gap-36">
                <div>
